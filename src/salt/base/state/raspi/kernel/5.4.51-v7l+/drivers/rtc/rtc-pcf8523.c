...
static int pcf8523_read(struct i2c_client *client, u8 reg, u8 *valuep)
{
	...
	if (err < 0) {
		dev_warn(&client->dev, "failed to read i2c register %d due to error %d\n", reg, err);
		return err;
	}

	// Uncomment below for debugging
	//dev_info(&client->dev, "read value %d from i2c register %d\n", value, reg);
...
static int pcf8523_write(struct i2c_client *client, u8 reg, u8 value)
{
	...
	if (err < 0) {
		dev_warn(&client->dev, "failed to write value %d to i2c register %d due to error %d\n", value, reg, err);
		return err;
	}

	// Uncomment below for debugging
	//dev_info(&client->dev, "wrote value %d to i2c register %d\n", value, reg);
...
static int pcf8523_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (err < 0) {
		dev_err(dev, "failed to read low battery voltage flag due to error %d\n", err);
		return err;
	} else if (err > 0) {
		dev_err(dev, "low battery voltage detected, time may be unreliable\n");
		// Continue anyway instead of disabling entire hwclock
		//return -EINVAL;
	}
	...
	err = i2c_transfer(client->adapter, msgs, ARRAY_SIZE(msgs));
	if (err < 0) {
		dev_err(dev, "failed to read time from i2c registers due to error %d\n", err);
		return err;
	}
	...
	if (regs[0] & REG_SECONDS_OS) {
		...
		if (err < 0) {
			dev_err(dev, "failed to toggle oscillator stopped/interrupted flag due to error %d\n", err);
			return err;
		}

		err = pcf8523_read(client, REG_SECONDS, &regs[0]);
		if (err < 0) {
			dev_err(dev, "failed to read oscillator stopped/interrupted flag due to error %d\n", err);
			return err;
		}

		if (regs[0] & REG_SECONDS_OS) {
			dev_err(dev, "unable to clear oscillator stopped/interrupted flag\n");
			return -EAGAIN;
		}

		dev_warn(dev, "cleared oscillator stopped/interrupted flag\n");
	}
...
static int pcf8523_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (tm->tm_year < 100 || tm->tm_year >= 200) {
		dev_warn(dev, "unable to set time because year is out of range\n");
		return -EINVAL;
	}

	err = pcf8523_stop_rtc(client);
	if (err < 0) {
		dev_err(dev, "failed to stop rtc due to error %d\n", err);
		return err;
	}
	...
	err = i2c_transfer(client->adapter, &msg, 1);
	if (err < 0) {
		dev_err(dev, "failed to write time to i2c registers due to error %d\n", err);
		/*
...
static int pcf8523_rtc_read_offset(struct device *dev, long *offset)
{
	...
	if (err < 0) {
		dev_err(dev, "failed to read offset from i2c register due to error %d\n", err);
		return err;
	}
