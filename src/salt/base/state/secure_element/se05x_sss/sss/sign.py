...
from .util import encode_signature, hash_convert, hash_convert_raw, save_to_file, \
...
class Sign:  # pylint: disable=too-few-public-methods
    ...
    def do_signature(self, key_id, input_file, filename,  # pylint: disable=too-many-locals, too-many-arguments
        ...
        :return: Status
        """
        status, object_type, cipher_type = self._ctx_key.get_handle(
            key_id)  # pylint: disable=unused-variable
        ...
        if hash_algo == '':
            ...
            if cipher_type in [apis.kSSS_CipherType_RSA, apis.kSSS_CipherType_RSA_CRT]:
                ...
                log.debug(
                    "Considering Algorithm_SSS_SHA512 as Default hash algorithm")
                self.hash_algo = apis.kAlgorithm_SSS_SHA512
            elif cipher_type == apis.kSSS_CipherType_HMAC:
                log.debug(
                    "Considering kAlgorithm_SSS_HMAC_SHA512 as Default hash algorithm")
                self.hash_algo = apis.kAlgorithm_SSS_HMAC_SHA512
            elif cipher_type == apis.kSSS_CipherType_AES:
                log.debug(
                    "Considering kAlgorithm_SSS_CMAC_AES as Default hash algorithm")
                self.hash_algo = apis.kAlgorithm_SSS_CMAC_AES
            elif cipher_type == apis.kSSS_CipherType_DES:
                log.debug(
                    "Considering kAlgorithm_SSS_DES_CMAC8 as Default hash algorithm")
                self.hash_algo = apis.kAlgorithm_SSS_DES_CMAC8
            else:
                log.debug(
                    "Considering Algorithm_SSS_SHA256 as Default hash algorithm")
        ...
        try:
            if not os.path.isfile(input_file):
                raise Exception(
                    "Incorrect input file, try signing with correct input file !!")
            ...
            if cipher_type in [apis.kSSS_CipherType_EC_TWISTED_ED]:
                ...
                (digest, digest_len) = hash_convert(
                    input_file, in_encode_format, self.hash_algo)
            ...
            if 'Unable to load certificate' in str(exc) or 'error parsing asn1 value' in str(exc):
                if os.path.isfile(input_file):
                    ...
                    if cipher_type in [apis.kSSS_CipherType_EC_TWISTED_ED]:
                        ...
                        (digest, digest_len) = hash_convert_raw(
                            src_data, self.hash_algo)
                else:
                    raise Exception(
                        "Incorrect input file, try signing with correct input file !!")  # pylint: disable=raise-missing-from
            else:
                raise exc
        ...
        if cipher_type in [apis.kSSS_CipherType_HMAC, apis.kSSS_CipherType_AES, apis.kSSS_CipherType_DES]:
            ...
            ctx = Asymmetric(self._session, self._ctx_key,
                             self.hash_algo, mode)
        if cipher_type in [apis.kSSS_CipherType_HMAC, apis.kSSS_CipherType_AES, apis.kSSS_CipherType_DES]:
            ...
            (signature_ctype, status) = ctx.se05x_sign(
                digest, digest_len, data_buf, signature_len)
        else:
            (signature_ctype, status) = ctx.sign(
                digest, digest_len, data_buf, signature_len)
    ...
    def do_signature_and_return(self, key_id, input_data, out_encode_format="PEM", hash_algo=""):
        ...
        if cipher_type in [apis.kSSS_CipherType_HMAC, apis.kSSS_CipherType_AES, apis.kSSS_CipherType_DES]:
            ...
            ctx = Asymmetric(self._session, self._ctx_key,
                             apis.kAlgorithm_SSS_SHA256 if self.hash_algo == "KECCAK256" else self.hash_algo, mode)
        if cipher_type in [apis.kSSS_CipherType_HMAC, apis.kSSS_CipherType_AES, apis.kSSS_CipherType_DES]:
            if self.symmetric_sign_mac_one_go:
                status = ctx.mac_one_go(self._ctx_key, digest, digest_len)
            else:
        ...
        encoded_data = encode_signature(
            signature_list, apis.kSSS_KeyPart_NONE, out_encode_format)
        return encoded_data

    # DIMO
    def sign_hash_and_return(self, key_id, hex_digest):
        """
        DO sign operation on a string and return the signature
        :param key_id: Key index
        :param hex_digest: String hex digest to be signed
        """
        status, object_type, cipher_type = self._ctx_key.get_handle(key_id)
        if status != apis.kStatus_SSS_Success:
            print("Failed to get key handle")
            return status
        _digest = bytearray.fromhex(hex_digest)
        digest_bytes = (ctypes.c_ubyte * len(_digest))(*_digest)
        mode = apis.kMode_SSS_Sign
        signature_len = 1024
        data_buf = (ctypes.c_uint8 * signature_len)(0)
        signature_len = ctypes.c_size_t(signature_len)
        ctx = Asymmetric(self._session, self._ctx_key,
                         apis.kAlgorithm_SSS_SHA256, mode)
        (signature_ctype, status) = ctx.sign(
            digest_bytes, len(digest_bytes), data_buf, signature_len)
        signature = binascii.hexlify(signature_ctype)
        return signature
