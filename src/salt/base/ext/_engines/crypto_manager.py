...
@edmp.register_hook()
def generate_key_handler(keyid):
    if not isinstance(keyid, str):
        keyid = format(keyid, '#x')
    
    with conn:
        log.info("Generating key: {}".format(keyid))

        key_obj = conn.generate_key(keyid)
        key_string = conn.get_public_key(key_obj)

        log.info(key_string)

        return { "public_key": key_string }


@edmp.register_hook()
def get_public_key_handler(keyid):   
    if not isinstance(keyid, str):
        keyid = format(keyid, '#x')
    
    with conn:
        key = conn.get_key_object(keyid)
        public_key = conn.get_public_key(key)

        log.info("Got public key {}".format(public_key))

        return { "public_key": public_key }

@edmp.register_hook()
def sign_string_handler(data, keyid):    
    if not isinstance(keyid, str):
        keyid = format(keyid, '#x')

    log.info("Executing Sign String on data: {}, {}".format(data, keyid))

    with conn:
        key = conn.get_key_object(keyid)
        signature = conn.sign_string(data, key)
